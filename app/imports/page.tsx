...
export default function ImportsPage() {
  ...
  const [smartSort, setSmartSort] = useState<'opportunity' | 'roi' | 'margin' | 'priceMove' | 'risk'>('opportunity');
  ...
  const fetchMarketData = async (showRefreshState = false) => {
    try {
      ...
      setError(null);
      // Reuses the existing market-intelligence API as the data engine for import decisions.
      ...
      if (!res.ok) {
        ...
        throw new Error(text || 'Nuk u arrit të lexohen të dhënat reale të importeve.');
      }
      ...
      console.error(err);
      setError(err instanceof Error ? err.message : 'Gabim gjatë ngarkimit të imports intelligence.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('sellsmartImportsWatchlist');
      if (saved) setWatchlist(JSON.parse(saved));
    } catch {
      localStorage.removeItem('sellsmartImportsWatchlist');
    }
  }, []);

  useEffect(() => {
    localStorage.setItem('sellsmartImportsWatchlist', JSON.stringify(watchlist));
  }, [watchlist]);
  ...
  useEffect(() => {
    ...
    if (onlyHighConfidence) {
      filtered = filtered.filter((item) => item.confidenceLevel >= 75 && item.dataQuality !== 'C');
    }

    const sorted = [...filtered].sort((a, b) => {
      if (smartSort === 'roi') return b.roi - a.roi;
      if (smartSort === 'margin') return b.marginPotential - a.marginPotential;
      if (smartSort === 'priceMove') return Math.abs(b.priceChange30d) - Math.abs(a.priceChange30d);
      if (smartSort === 'risk') return a.competitorCount + Math.abs(a.priceChange30d) - (b.competitorCount + Math.abs(b.priceChange30d));
      return b.opportunityScore - a.opportunityScore;
    });

    setFilteredData(sorted);
  }, [selectedCategory, selectedRegion, selectedRisk, selectedAction, marketData, searchTerm, onlyHighConfidence, smartSort]);
  ...
  const watchlistItems = useMemo(() => {
    return marketData.filter((item) => watchlist.includes(item.id));
  }, [marketData, watchlist]);

  const executiveSummary = useMemo(() => {
    const highConfidence = filteredData.filter((item) => item.confidenceLevel >= 75).length;
    const highRisk = filteredData.filter((item) => item.riskLevel === 'High').length;
    const premiumCandidates = filteredData.filter((item) => item.marginPotential >= averageMargin && item.roi >= averageRoi).length;
  ...
  }, [filteredData, averageMargin, averageRoi, watchlistItems.length]);

  const resetFilters = () => {
    setSelectedCategory('Të gjitha');
    setSelectedRegion('Të gjitha');
    setSelectedRisk('Të gjitha');
    setSelectedAction('Të gjitha');
    setSearchTerm('');
    setOnlyHighConfidence(false);
    setSmartSort('opportunity');
  };
  ...
  const exportCsv = () => {
    ...
    a.href = url;
    a.download = `sellsmart-imports-intelligence-${timeRange}.csv`;
    a.click();
      ...
      <AuthGuard>
        <AppShell>
          <div className="grid min-h-[70vh] place-items-center">
            <div className="w-full max-w-3xl rounded-[36px] border border-white/70 bg-white/80 p-8 text-center shadow-[0_24px_90px_rgba(15,23,42,0.10)] backdrop-blur-2xl">
              <div className="spinner mx-auto mb-5"></div>
              <h2 className="text-3xl font-black tracking-[-0.04em] text-slate-950">Duke ngarkuar imports intelligence...</h2>
              <p className="mt-3 text-sm font-medium text-slate-500">Po lexohen sinjalet reale, konkurrentët, rajonet, risku dhe oportunitetet.</p>
              <div className="mt-7 grid grid-cols-1 gap-3 sm:grid-cols-3">
                {['Burime reale', 'Çmime', 'Konkurrentë'].map((item) => (
                  <div key={item} className="h-24 animate-pulse rounded-3xl bg-slate-100">
                    <div className="p-4 text-xs font-black uppercase tracking-wider text-slate-300">{item}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <AppShell>
        <div className="space-y-8">
              ...
              <div>
                <div className="flex flex-wrap items-center gap-3 mb-6">
                  <span className="px-4 py-2 rounded-full bg-gradient-to-r from-indigo-500/15 to-cyan-500/15 text-indigo-700 font-semibold text-sm border border-indigo-200/50">
                    Import Intelligence • Të dhëna reale
                  </span>
                  <span className="px-4 py-2 rounded-full bg-white/70 text-slate-700 font-semibold text-sm border border-white/70">
                    {filteredData.length} sinjale aktive
                  </span>
                  <span className="px-4 py-2 rounded-full bg-emerald-50 text-emerald-700 font-semibold text-sm border border-emerald-100">
                    {executiveSummary.highConfidence} high-confidence
                  </span>
                </div>
                ...
                <p className="mt-4 max-w-3xl text-slate-600 text-lg leading-8">
                  Kontroll premium për importet, furnizimin, marzhën, rrezikun e stokut dhe mundësitë reale të blerjes nga tregu.
                </p>
              </div>

              <div className="glass-card rounded-3xl p-6 w-full xl:w-[460px]">
                <div className="flex items-center justify-between mb-3">
                  <div className="text-xs font-semibold text-indigo-600 uppercase tracking-[0.18em]">Pulsi i Importeve</div>
                  <span
                ...
                <div className="flex items-end justify-between gap-4">
                  <div>
                    ...
                    <div className="text-sm text-slate-500">Forca e sinjalit për import</div>
                  </div>
                  <button
          ...
          <section className="grid grid-cols-1 gap-5 xl:grid-cols-12">
            <div className="xl:col-span-8 rounded-[32px] border border-white/70 bg-slate-950 p-6 text-white shadow-[0_24px_90px_rgba(15,23,42,0.20)]">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <div className="text-xs font-black uppercase tracking-[0.2em] text-cyan-200">Executive command center</div>
                  <h2 className="mt-2 text-3xl font-black tracking-[-0.04em]">Vendimi i radhës nga të dhënat reale</h2>
                  <p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">
                    Filtro sinjalet me cilësi të lartë, eksportoji në CSV, mbaj watchlist dhe fokuso vetëm oportunitetet që kanë marzhë, ROI dhe risk të pranueshëm.
                  </p>
                </div>
                <div className="flex flex-wrap gap-3">
                  <button
                    type="button"
                    onClick={() => setOnlyHighConfidence((prev) => !prev)}
                    className={`rounded-2xl px-4 py-3 text-sm font-black transition-all ${onlyHighConfidence ? 'bg-cyan-400 text-slate-950' : 'bg-white/10 text-white hover:bg-white/15'}`}
                  >
                    {onlyHighConfidence ? '✓ High confidence ON' : 'High confidence'}
                  </button>
                  <button
                    type="button"
                    onClick={exportCsv}
                    className="rounded-2xl bg-white px-4 py-3 text-sm font-black text-slate-950 transition-all hover:-translate-y-0.5"
                  >
                    Eksporto CSV
                  </button>
                  <button
                    ...
                    className="rounded-2xl bg-white/10 px-4 py-3 text-sm font-black text-white transition-all hover:bg-white/15"
                  >
                    Reset filters
                  </button>
                </div>
              ...
              {commandCenterOpen && (
                <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
                  {[
                    ['High confidence', executiveSummary.highConfidence, 'Signals with better data quality'],
                    ['High risk', executiveSummary.highRisk, 'Needs careful pricing'],
                    ['Premium candidates', executiveSummary.premiumCandidates, 'Good margin + ROI'],
                    ['Watchlist', executiveSummary.watchlistCount, 'Saved opportunities'],
                  ].map(([label, value, note]) => (
                    <div key={String(label)} className="rounded-3xl bg-white/10 p-4 ring-1 ring-white/10">
              ...
              <button
                ...
                className="mt-4 text-xs font-black uppercase tracking-wider text-cyan-200 hover:text-white"
              >
                {commandCenterOpen ? 'Collapse command center' : 'Open command center'}
              </button>
            </div>

            <div className="xl:col-span-4 rounded-[32px] border border-white/70 bg-white/80 p-6 shadow-[0_22px_80px_rgba(15,23,42,0.08)] backdrop-blur-2xl">
              <div className="text-xs font-black uppercase tracking-[0.18em] text-indigo-600">Smart sorting</div>
              <h3 className="mt-1 text-2xl font-black tracking-[-0.04em] text-slate-950">Prioritizo sinjalet</h3>
              <select
                value={smartSort}
                onChange={(e) => setSmartSort(e.target.value as typeof smartSort)}
                className="mt-5 w-full rounded-2xl border border-slate-200 bg-white/80 px-4 py-3 text-sm font-black text-slate-700 outline-none focus:border-indigo-300 focus:ring-4 focus:ring-indigo-100"
              >
                <option value="opportunity">Opportunity score</option>
                <option value="roi">ROI më i lartë</option>
                <option value="margin">Marzha më e lartë</option>
                <option value="priceMove">Lëvizja më e fortë e çmimit</option>
                <option value="risk">Risk më i ulët</option>
              </select>
              <div className="mt-5 rounded-2xl bg-indigo-50 p-4 text-sm leading-6 text-slate-600">
                Smart sort ndikon direkt në tabelat dhe oportunitetet që shfaqen më poshtë.
              </div>
            </div>
          </section>

          {watchlistItems.length > 0 && (
            <section className="rounded-[32px] border border-indigo-100 bg-gradient-to-br from-indigo-50 to-cyan-50 p-6 shadow-[0_22px_80px_rgba(79,70,229,0.10)]">
              <div className="mb-4 flex items-center justify-between gap-4">
                <div>
                  <div className="text-xs font-black uppercase tracking-[0.18em] text-indigo-700">Watchlist</div>
                  <h3 className="text-2xl font-black tracking-[-0.04em] text-slate-950">Sinjalet që po i monitoron</h3>
                </div>
                <button onClick={() => setWatchlist([])} className="rounded-2xl bg-white/80 px-4 py-2 text-xs font-black text-slate-600 hover:bg-white">Pastro</button>
              </div>
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                {watchlistItems.slice(0, 4).map((item) => (
                  <div key={item.id} className="rounded-3xl bg-white/80 p-4 shadow-sm">
                    <div className="text-sm font-black text-slate-950">{item.productName}</div>
                    <div className="mt-1 text-xs text-slate-500">{item.category} • {item.region}</div>
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-xl font-black text-indigo-600">€{item.currentPrice}</span>
                      <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600">{item.recommendedAction}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-4 mb-6">
            <div className="flex flex-wrap gap-4">
              ...
              <button
                type="button"
                onClick={() => setOnlyHighConfidence((prev) => !prev)}
                className={`rounded-2xl px-4 py-3 text-sm font-black transition-all ${onlyHighConfidence ? 'bg-slate-950 text-white shadow-lg' : 'glass-card text-slate-700 hover:bg-white/90'}`}
              >
                {onlyHighConfidence ? '✓ Cilësi e lartë' : 'Cilësi e lartë'}
              </button>
            </div>
            ...
            <div className="glass-card rounded-2xl px-5 py-4 flex flex-wrap items-center gap-4">
              ...
              <div>
                <div className="text-xs text-slate-500">Vlera e importeve</div>
                        ...
                        <tr key={signal.id} className="hover:bg-white/40 transition-colors">
                          <td className="px-6 py-4">
                            <button
                              ...
                              className={`rounded-full px-3 py-1 text-xs font-black transition-all ${watchlist.includes(signal.id) ? 'bg-indigo-600 text-white' : 'bg-white/70 text-slate-500 hover:bg-indigo-50 hover:text-indigo-700'}`}
                            >
                              {watchlist.includes(signal.id) ? '✓ Saved' : '+ Save'}
                            </button>
                          </td>
      ...
      <div className="bg-white/50 rounded-xl p-3">
        ...
        <span className="font-bold ml-1">{region.supplyIndex}</span>
                  ...
                  {competitors.map((comp) => (
                    <div key={comp.id} className="bg-white/55 rounded-2xl p-4 hover:bg-white/80 transition-all">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm mb-3">
                        ...
                        <div className="bg-white/60 rounded-xl p-3">
                          <div className="text-slate-500 text-xs">Pjesa e importeve</div>
          ...
          <div className="mt-8 text-center text-xs text-slate-400">
            SellSmart Imports Intelligence • {filteredData.length} sinjale aktive • {watchlist.length} në watchlist • CSV export gati • Të dhënat rifreskohen nga burime reale
          </div>
        </div>
      </AppShell>
    </AuthGuard>
