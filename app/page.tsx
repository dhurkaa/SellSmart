...
export default function CompanyHome() {
  ...
  const handleBaseChange = (
    ...
    if (error) {
      setFieldErrors((prev) => [...prev, { field: name, message: error }]);
    }
  };
  ...
  const callAssistant = async (nextMessages: ChatMessage[]) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        setSessionError({ type: 'timeout', message: 'Timeout i rrjetit. Kontrollo lidhjen.' });
  ...
  const retryLastRequest = async () => {
    ...
    // Retry with current messages
    await callAssistant(messages);
  };
    ...
    validFiles.forEach((file, index) => {
      ...
      reader.onload = () => {
        ...
        setUploadedImages((prev) => {
          ...
          // Limit to max count
          if (newImages.length > VALIDATION_LIMITS.imageMaxCount) {
            setError(`Maksimumi ${VALIDATION_LIMITS.imageMaxCount} imazhe mund të ngarkohen.`);
            return newImages.slice(0, VALIDATION_LIMITS.imageMaxCount);
          }
          return newImages;
        });
      };
              ...
              <div className="glass-card rounded-2xl p-6 w-full xl:w-[400px] transition-all hover:shadow-lg">
                ...
                <div className="grid grid-cols-2 gap-4 mt-6">
                  <div className="bg-white/50 rounded-xl p-3 text-center backdrop-blur-sm">
                    ...
                    <div className="text-2xl font-black text-slate-900">{uploadedImages.length}/{VALIDATION_LIMITS.imageMaxCount}</div>
                  </div>
                  <div className="bg-white/50 rounded-xl p-3 text-center backdrop-blur-sm">
                  ...
                  <form onSubmit={startSession} className="space-y-8">
                    ...
                    <label className="block">
                      <span className="text-xs font-semibold text-slate-500 uppercase">
                        Emri i produktit * <span className="font-normal text-slate-400">({baseInfo.productName.length}/{VALIDATION_LIMITS.productName.max})</span>
                      </span>
                      <input
                        name="productName"
                    ...
                    <label className="block">
                      <span className="text-xs font-semibold text-slate-500 uppercase">
                        Lokacioni * <span className="font-normal text-slate-400">({baseInfo.location.length}/{VALIDATION_LIMITS.location.max})</span>
                      </span>
                      <input
                        name="location"
                        ...
                        >
                          <div className="text-slate-600">
                            ...
                            <label className="inline-flex mt-6 cursor-pointer">
                              ...
                              />
                              <span className={`px-6 py-2 rounded-full font-semibold text-sm shadow-sm transition-all ${
                ...
                <div className="space-y-6">
                  {valuation ? (
                    <div className="glass-card-strong rounded-3xl p-6 md:p-8 lg:p-10">
                      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
                        ...
                        <button onClick={handleExport} className="px-4 py-2 rounded-full bg-indigo-50 text-sm font-semibold text-indigo-600 shadow-sm hover:bg-indigo-100 transition-all flex items-center gap-2">
                          📄 EKSPORTO RAPORTIN
                        </button>
                      </div>
                  ...
                  <div className="glass-card rounded-3xl p-5 md:p-6 lg:p-7">
                    <div className="flex flex-wrap items-center justify-between gap-4 mb-5">
                      ...
                      <button onClick={resetSession} className="px-4 py-2 rounded-full bg-white/80 text-sm font-medium text-slate-600 hover:bg-white transition-all flex items-center gap-2" disabled={loading}>
