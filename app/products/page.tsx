...
const CategoryBadge = ({ category }: { category: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${cat?.color || "bg-gray-100 text-gray-800"}`}
    >
      <span>{cat?.icon || "📦"}</span>
  ...
  <div className="glass-card rounded-3xl p-12 text-center">
    ...
    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
      <button
        type="button"
        onClick={onReset}
        className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/80 text-slate-700 font-semibold shadow-sm hover:bg-white transition-all"
      >
        <span>✖️</span>
        Pastro filtrat
      </button>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-slide-in">
      <div className="glass-card-strong rounded-3xl max-w-md w-full">
        <div className="p-6 text-center">
          ...
          <p className="text-slate-500 mb-6">
            A jeni i sigurt që dëshironi ta fshini produktin "
            <strong>{productName}</strong>"? Ky veprim nuk mund të zhbëhet.
          </p>
...
export default function ProductsPage() {
  ...
  const calculateStats = (productsData: Product[]) => {
    ...
    const averagePrice =
      productsData.length > 0 ? totalValue / productsData.length : 0;
    ...
    productsData.forEach((p) => {
      const month = new Date(p.created_at).toLocaleDateString("sq-AL", {
        year: "numeric",
        month: "short",
      });
    ...
    <AppShell>
      ...
      <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              ...
              <p className="mt-2 text-slate-600 max-w-2xl">
                Menaxho të gjithë inventarin, ndiq performancën dhe optimizo
                çmimet me inteligjencën artificiale.
              </p>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            title="Kategoritë"
            value={Object.keys(stats.categoriesCount).length || 0}
            icon="🏷️"
            color="bg-purple-100"
          />
        </div>
